...
let loginUser = async function (req, res) {
    try {
        ...
        if (!Email) return res.status(400).send({ status: false, message: "email is not correct" })
        //---------[Check Password]
        let user = await userModel.findOne({ email: email, password: password })
        if (!user) return res.status(400).send({ status: false, message: "password is not corerct" });
